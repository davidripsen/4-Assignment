...
## Plot power vs (normalized) time-of-year
...
This findings will be utilised in the modelling section later.
...
\begin{enumerate}
\item The wind speed seem to affect the power in a s-curve, i.e. use a logistic function on the wind speed, i.e. $p = ... + b\cdot\frac{e^{a \cdot WindSpeed}}{1+e^{a \cdot WindSpeed}}$
...
# Fit MSM
fit_msm = msmFit(fit2b, k=2, p=1, sw=rep(TRUE,6), control = list(parallel = TRUE))
summary(fit_msm)
...
Dsub = D[1:1000, ]
fit2bsub = lm(p ~ Ws1 + Wd1 + ntoy, data=Dsub)
fit_msm_sub = msmFit(fit2bsub, k=2, p=1, sw=rep(TRUE,6), control = list(parallel = TRUE))
plotProb(fit_msm_sub, which=2)
```
...
fit_msm2_sub = msmFit(fit2bsub, k=3, p=1, sw=rep(TRUE,6), control = list(parallel = TRUE))
for (i in 2:4){
plotProb(fit_msm2_sub, which=i)
}
```
...
#### evt. Discrete State Space Form / Extended Kalman Filter (with adaptivity) ####
